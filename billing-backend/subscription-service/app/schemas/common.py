...
class EventIdPath(BaseModel):
    """Path parameter validation for event ID."""
    
    event_id: str = Field(..., min_length=1, max_length=100, description="Event ID string")


class FeatureNamePath(BaseModel):
    """Path parameter validation for feature name."""
    
    feature_name: str = Field(..., min_length=1, max_length=50, pattern=r"^[a-zA-Z0-9_]+$", description="Feature name (alphanumeric and underscores)")
...
class SubscriptionSortBy(str, Enum):
...
class SubscriptionFilterParams(BaseModel):
    ...
    status: Optional[str] = Field(None, pattern=r"^(pending|active|trial|past_due|cancelled|revoked)$")
    plan_id: Optional[int] = Field(None, description="Filter by plan ID")
    is_trial: Optional[bool] = Field(None, description="Filter by trial status")
    
    # Date range filters
    start_date_from: Optional[str] = Field(None, pattern=r"^\d{4}-\d{2}-\d{2}$", description="Start date from (YYYY-MM-DD)")
    start_date_to: Optional[str] = Field(None, pattern=r"^\d{4}-\d{2}-\d{2}$", description="Start date to (YYYY-MM-DD)")
    
    # Sorting
    sort_by: SubscriptionSortBy = Field(default=SubscriptionSortBy.CREATED_AT)
...
class UsageFilterParams(BaseModel):
    ...
    feature_name: Optional[str] = Field(None, max_length=50, description="Filter by feature name")
    
    # Date range filters
    updated_from: Optional[str] = Field(None, pattern=r"^\d{4}-\d{2}-\d{2}$", description="Updated date from (YYYY-MM-DD)")
    updated_to: Optional[str] = Field(None, pattern=r"^\d{4}-\d{2}-\d{2}$", description="Updated date to (YYYY-MM-DD)")
    
    # Sorting
    sort_by: UsageSortBy = Field(default=UsageSortBy.UPDATED_AT)
    sort_order: SortOrder = Field(default=SortOrder.DESC)
